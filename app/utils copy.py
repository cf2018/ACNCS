...
def process_image(image_path):
    ...
    even if the contour is not solid, by joining the dots.
    """
    # Load the image
    ...
    # Apply morphological closing and dilation to fill gaps in dotted lines
    ...
    for contour in contours:
        if len(contour) >= 5:
            # Get the convex hull to ensure a continuous curve
            ...
            # Clip values to ensure they stay within image bounds
            ...
            # Draw the green start-to-end line
            x1, y1 = x_fit[0], y_fit[0]
            x2, y2 = x_fit[-1], y_fit[-1]
            cv2.line(img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
            
            # Compute the perpendicular blue line
            ...
            # Calculate percentages and display text
            ...
            text_x = int(max(intersection_x, x_p) + 20)
            text_y = int((intersection_y + y_p) / 2)
            
            cv2.putText(img, f"{green_percentage:.1f}% along green line", (text_x, text_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
            cv2.putText(img, f"Blue/Red Length: {blue_red_ratio:.1f}%", (text_x, text_y + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
...
def process_image_solid(image_path):
    """
    Detects and labels the curved yellow lines in the sail image.
    """
    # Load the image
...
if __name__ == "__main__":
    image_path = "sail_image.jpg"  # Replace with your image path
    try:
